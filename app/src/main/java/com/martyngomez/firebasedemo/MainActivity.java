...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvDescountMessage.setVisibility(View.GONE); //Oculta un view



        String token = FirebaseInstanceId.getInstance().getToken(); //Resgistra y Obtiene Token
        Log.w(TAG, "token: " + token);

        if (getIntent().getExtras() != null){
            tvDescountMessage.setVisibility(View.VISIBLE); // Hace visible un View
            String descount = getIntent().getExtras().getString(KEY_DESCOUNT);
            tvDescountMessage.append(descount); //append Concatena texto
        }
